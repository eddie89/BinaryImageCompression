...
public class PictureLauncher extends Application {
	...
	public void drawImage(List<Block> blockList, String fileName, WritableImage writableImage) {
		...
		File file = new File("resultPictures/GeneralizedDelta_" + fileName);
		...
		System.out.println("GeneralizedDelta Blocks: " + blockList.size());
	}
	...
	public void saveImages(String[] args) {
		...
		try (DirectoryStream<Path> stream = Files.newDirectoryStream(directory)) {
			for (Path file : stream) {
				...
				Map<Integer, List<Row>> rowMap = segmentator.computeRows(image);
				Map<List<Integer>, Set<Integer>> blockMap = segmentator.computeBlocks(rowMap);
				List<Block> blockList = segmentator.buildBlocks(blockMap);
